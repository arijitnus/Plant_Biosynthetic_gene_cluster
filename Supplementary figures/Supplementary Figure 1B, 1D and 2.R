...
#Fig. S1B for genome size, completeness and contamination across datasets
library(readxl)
library(dplyr)
library(ggplot2)
all_soil_mags<-read_excel("/Users/arijitmukherjee/Documents/Phytobiome/BGC_isolates/New_final_data/MAGs/Earth_MAGs/earthmicrobiome_bgc_data (1).xlsx",col_names = T,skip = 0,sheet = "soil_bgc")
...
dim(all_plant_mags)
plant_isolates<-read_excel("/Users/arijitmukherjee/Documents/Phytobiome/BGC_isolates/New_final_data/Network_Annotations_Full_all_isolates.xlsx",sheet = "Filtered",col_names = T,skip = 0)
soil_isolates<-read_excel("/Users/arijitmukherjee/Downloads/genome_quality_info_all_soil_isolates.xlsx",sheet = "Sheet1",col_names = T,skip = 0)


df$Data<-factor(df$Data,levels=c("Plant isolates","Soil isolates","Plant MAGs","Soil MAGs"))

completeness_df<-data.frame(Datasets=c(rep("Plant isolates",2761),rep("Plant MAGs",1453),rep("Soil MAGs",1300),rep("Soil isolates",2557)),
                            completeness=c(plant_isolates$Completeness,all_plant_mags$Completeness,all_soil_mags$completeness,soil_isolates$completeness))
completeness_df$Datasets<-factor(completeness_df$Datasets,levels = c("Plant isolates","Soil isolates","Plant MAGs","Soil MAGs"))
...
##Plot the contamination distributions for genomes across datasets
plant_isolates$Contamination
contam_df<-data.frame(Datasets=c(rep("Plant_isolates",2761),rep("Plant_MAGs",1453),rep("Soil_MAGs",1300),rep("Soil isolates",2549)),
                      contam=c(plant_isolates$Contamination,all_plant_mags$Contamination,all_soil_mags$contamination,soil_isolates$contamination))
...
GS_df<-data.frame(Datasets=c(rep("Plant isolates",2761),rep("Plant MAGs",1453),rep("Soil MAGs",1300),rep("Soil isolates",2557)),
                  Genome_size=c(plant_isolates$Genome_Size/10^6,all_plant_mags$`Genome_size (bp)`/10^6,all_soil_mags$genome_length/10^6,soil_isolates$length/10^6))# Genome size has been converted into Mbp

GS_df$Datasets<-factor(GS_df$Datasets,levels = c("Plant isolates","Soil isolates","Plant MAGs","Soil MAGs"))
